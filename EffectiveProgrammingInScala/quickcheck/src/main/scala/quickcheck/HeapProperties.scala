...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val larger = math.max(x, y)
      val meldedHeap: List[Node] = meld(
        insert(smaller, insert(smaller, empty)),
        insert(larger, insert(larger, empty))
      )
      ...
      val deleteTwoMinAndFindMin: Boolean =
        findMin(deleteMin(deleteMin(meldedHeap))) == larger
      ...
      val insertMinAndFindMin: Boolean =
        findMin(insert(smaller, meldedHeap)) == smaller
  ...
  val meldingHeaps: (String, Prop) =
    def check(meldedHeap: List[Node], heap1: List[Node], heap2: List[Node]): Boolean =
      if isEmpty(heap1) && isEmpty(heap2) && isEmpty(meldedHeap) then
        ...
        if !isEmpty(heap1) && findMin(heap1) == minOfMelded then
          check(deleteMin(meldedHeap), deleteMin(heap1), heap2)
        
        else if !isEmpty(heap2) && findMin(heap2) == minOfMelded then
