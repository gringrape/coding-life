...
object EndScene extends Scene[MyStartupData, GameModel, ViewModel] {
  ...
  override def updateModel(
    ...
    case KeyboardEvent.KeyDown(Key.ENTER) =>
      Outcome(model)
        .addGlobalEvents(SceneEvent.JumpTo(SceneName("start")))
  ...
  override def name: SceneName = SceneName("end")
  ...
  def drawTitle(): TextBox =
    ...
    TextBox("게임종료", textWidth, textHeight)
  ...
  def drawStartButton() =
    ...
    val textBox = TextBox("다시 시작", textWidth, textHeight)
      .withColor(RGBA.Black)
      .withFontSize(Pixels(fontSize))
      .withPosition(
        Point(
